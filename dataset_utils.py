...
def get_mnist_dataloaders(batch_size=64, data_dir='./data'):
	...
	print(f"MNIST dataset loaded. Training samples: {len(train_dataset)}, Test samples: {len(test_dataset)}, Number of workers: {multiprocessing.cpu_count()}")
	print(f"Train Dataloader batch size: {batch_size}, Test Dataloader batch size: {batch_size}, Number of workers: {multiprocessing.cpu_count()}")
...
if __name__ == '__main__':
	...
	print(f"Images batch shape: {images.shape}") # Expected: [batch_size, 1, 28, 28] for MNIST
